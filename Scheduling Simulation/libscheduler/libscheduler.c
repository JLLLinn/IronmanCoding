...
int scheduler_new_job(int job_number, int time, int running_time, int priority)
{
	...
	if(schemeType == PPRI){
		...
		for(i = 0; i<coreNumber; i++){//look for idle core, meanwhild go throught every core to find the worst job (if any)
			...
			if(worstCoreNumber == -1){//first time and new job's priority is higher
				...
				if(p == worstCorePri){//if it's samely worse
					...
					if(corelistById[i].curJobPointer->arrivial > corelistById[worstCoreNumber].curJobPointer->arrivial){
						worstCoreNumber = i;
						worstCorePri = p;
					}
				}
		...
		if(worstCoreNumber != -1){
			...
			coreJob->remaining = coreJob->remaining - (time - coreJob->lastTimeInCore);
	...
	if(schemeType == PSJF){
			...
			for(i = 0; i<coreNumber; i++){//look for idle core, meanwhild go throught every core to find the worst job (if any)
				...
				//if this core is not idle and if it's the first time finding a job
				int remain = corelistById[i].curJobPointer->remaining - (time - corelistById[i].curJobPointer->lastTimeInCore);
				//printf("Hey I'm here!\n");
...
int scheduler_job_finished(int core_id, int job_number, int time)
{
	...
	corelistById[core_id].waiting += (curJob->leave - curJob->arrivial - curJob->duration);
