...
void readFile(int *lnCount, int** inputArray){
	...
	while(1){
		...
		if(line_ct<=0)
			break;
		if(curSize<=(*lnCount)){
			(*inputArray) = (int*)realloc(*inputArray, sizeof(int)*2*curSize);
			curSize*=2;
		}
...
int main(int argc, char **argv)
{
	...
	if (input_ct % segment_count == 0){
	    ...
	    values_last_segment = input_ct - values_per_segment * (segment_count-1);
	...
	struct qsort_thread_info *qinfo = malloc(segment_count*sizeof(struct qsort_thread_info));
	...
	while(1){
		...
		p2Segs2=(int**)realloc(p2Segs2,sizeof(int*)*(segment_count/2+1));
		minfo = realloc(minfo,(segment_count/2)*sizeof(struct mergesort_thread_info));
		...
		segment_count= (segment_count%2==1)?(segment_count/2+1):(segment_count/2);
