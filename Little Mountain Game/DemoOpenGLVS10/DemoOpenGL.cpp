...
{
	float tests[] = {a[1]*b[2]-a[2]*b[1], a[2]*b[0]-a[0]*b[2], a[0]*b[1]-a[1]*b[0]};
...
{
    float x = vec1[0]; float y = vec1[1]; float z = vec1[2];
    float i = vec2[0]; float j = vec2[1]; float k = vec2[2];
...
void init(void) 
{
	...
	lookAt=new float[3];
	up=new float[3];
	eye[0]=0.0;eye[1]=0.0;eye[2]=0.25;
	lookAt[0]=0.5;lookAt[1]=0.5;lookAt[2]=0.0;
...
void display(void)
{
    ...
    gluLookAt(eye[0], eye[1], eye[2], eye[0]+lookAt[0], eye[1]+lookAt[1], eye[2]+lookAt[2], up[0], up[1], up[2]);
